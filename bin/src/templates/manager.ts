...
export function findTemplatePath(templateLanguage: 'ts' | 'js', __dirname: string, templateAuthMethod: AuthMethod): string {
  ...
  const templatePaths = [
    ...
    path.join(__dirname, '..', '..', 'templates', templateLanguage, templateAuthMethod), // Standard build: dist/bin -> dist/templates
    path.resolve(__dirname, '..', '..', 'templates', templateLanguage, templateAuthMethod), // Absolute path for npx

    // Global npm installation paths
    path.join(__dirname, '..', '..', '..', 'templates', templateLanguage, templateAuthMethod), // npm global install
    path.join(process.env.NODE_PATH || '', 'create-tin', 'dist', 'templates', templateLanguage, templateAuthMethod),

    // Local npm installation paths
    path.join(process.cwd(), 'node_modules', 'create-tin', 'dist', 'templates', templateLanguage, templateAuthMethod),
    path.join(process.cwd(), 'node_modules', 'create-tin', 'templates', templateLanguage, templateAuthMethod),

    // Development mode paths (keep these last)
    path.join(__dirname, '..', 'templates', templateLanguage, templateAuthMethod), // Same directory as script
    path.join(process.cwd(), 'templates', templateLanguage, templateAuthMethod), // Current working directory
  ];
